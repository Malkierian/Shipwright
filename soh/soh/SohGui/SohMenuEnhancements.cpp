...
static bool allItemCountsChecked = false;

void AllItemCounts(WidgetInfo& info) {
    int numOptions = ARRAY_COUNT(itemCountMessageCVars);
    allItemCountsChecked = std::all_of(itemCountMessageCVars, itemCountMessageCVars + numOptions,
        [](const char* cvar) { return CVarGetInteger(cvar, 0); });
}
static bool allSkipsChecked = false;

void SkipAllCutscenes(WidgetInfo& info) {
    allSkipsChecked =
